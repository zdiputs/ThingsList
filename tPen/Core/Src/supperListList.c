...
LISTLIST_FRAME sppuerListListA[ListListANum] = {
    {"A步骤1", 0, 100, 1}, // A1步骤记录体
    {"A步骤2", 0, 100, 2}, // A2步骤记录体
    {"A步骤3", 0, 100, 0}  // A2步骤记录体

};
// A事情中 的3行记录分别是三个步骤
LISTLIST_FRAME sppuerListListB[ListListBNum] = {
    {"B步骤1", 0, 100, 1}, // B1步骤记录体
    {"B步骤2", 0, 100, 2}, // B2步骤记录体
    {"B步骤3", 0, 100, 0}  // B3步骤记录体

};
SUPPER_FRAME sppuerList[supperNum] =
{
  {1, 1, 0, "事情A", supperTaskA,&sppuerListListA},
  {1, 1, 0, "事情B", supperTaskB,&sppuerListListB}
...
signed short supperTaskA(unsigned char i, signed short Config, signed int Stay)
{
  ...
  //因为固定为事情A这个i是确定的
  if (sppuerListListA[j].StepSegFlag == STEP_PRE) //环节Pre
  {
    sppuerListListA[j].StepSegFlag = STEP_POLL;
  }

  if (sppuerListListA[j].StepSegFlag == STEP_POLL) //环节Poll
  {
    if (SupMTimeCnt > sppuerListListA[j].internalCntMax)
    {
      SupMTimeCnt = 0;
      SupPollNumSelfSub;
      if (SupJumpIsEn) //可以跳转非驻留
      {
        SupMState = sppuerListListA[j].nextIndex;//跳转
...
signed short supperTaskB(unsigned char i, signed short Config, signed int Stay)
{
  ...
  //每一个步骤都有跳转函数事先放在listlist中
  //因为固定为事情B这个i是确定的
  if (sppuerListListB[j].StepSegFlag == STEP_PRE) //环节Pre
  {
    sppuerListListB[j].StepSegFlag = STEP_POLL;
  }

  if (sppuerListListB[j].StepSegFlag == STEP_POLL) //环节Poll
  {
    if (SupMTimeCnt > sppuerListListB[j].internalCntMax)
    {
      SupMTimeCnt = 0;
      //这里放
      SupPollNumSelfSub;
      if (SupJumpIsEn) //可以跳转非驻留
      {
        SupMState = sppuerListListB[j].nextIndex;//跳转
        sppuerListListB[*j].StepSegFlag = STEP_PRE; //下一个步骤的环节的复位
      }
    }
  }
  return Config;
